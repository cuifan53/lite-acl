export default class LiteAcl {
	...
	private rolePermissions: Map<string, Set<string | number>> = new Map();
	...
	add(
		role: string | { [key: string]: (string | number)[] },
		...
		if (Object.prototype.toString.call(role) === "[object String]") {
			this.rolePermissions.set(
				role as string,
				new Set([
					...(this.rolePermissions.get(role as string) || []),
			...
			for (const k of Object.keys(role)) {
				this.rolePermissions.set(
					k,
					new Set([...(this.rolePermissions.get(k) || []), ...role[k]])
				);
			}
		}
	}
	...
	view(
		role: string | string[] = []
	): { role: string; permissions: (string | number)[] }[] {
	 ...
	 */
	can(
		askRole: string | string[],
		askPermission: string | number | (string | number)[],
		options?: { canType?: "all" | "one" }
	): boolean {
		const askRoles = Array.isArray(askRole) ? askRole : [askRole];
		const askPermissions = Array.isArray(askPermission)
			? new Set([...askPermission])
			: new Set([askPermission]);
 ...
 * @param role
 */
export const Can = function(
	permission: string | number | (string | number)[],
	role: string | string[] = "",
	options: { canType?: "all" | "one" } = { canType: "all" }
): Function {
	return function(
		target: any,
		propertyKey: string,
		...
		descriptor.value = function(...args: any[]) {
			...
			if (!LiteAcl.getAC().can(askRole, permission, options)) {
